...
function App() {
    const [user, setUser] = useState(localStorage.getItem('anicomplete-user') ? JSON.parse(localStorage.getItem('anicomplete-user')) :{username: '', id: 0, theme: 'default'})
    ...
    function getList() {
        ...
        fetch(url, options)
            ...
            .then((data) => {
                setData(data.data.MediaListCollection.lists.find(l => l.name === "Completed")?.entries || []);
                setLoading(false);
            })
            .catch(console.log);
    }
    ...
    return (
        <div>
            ...
            <p style={{color: color}} className="mb-2">Total mismatches: {data.filter(e => e.progress != e.media.chapters).length}</p>
            {loading && <p>Loading...</p>}
            {data.filter(e => e.progress !== e.media.chapters)
                .sort((e1, e2) => (e1.media.title.english || e1.media.title.romaji || '').localeCompare(e2.media.title.english || e2.media.title.romaji || ''))
                .map((item) => {
                    return (
                        <div className="w-full flex justify-center items-center m-2">
                            <a className="w-2/3 min-w-xs" href={item.media.siteUrl} key={item.media.title} target="_blank" rel="noopener noreferrer">
                                <Card className="flex flex-row gap-3 justify-center items-center p-2">
                                    <img src={item.media.coverImage.large} alt={item.media.title} style={{ height: '60px', width: '50px' }} />
